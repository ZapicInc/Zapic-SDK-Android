...
public final class Alerter {
     ...
     */
    public static void clearCurrent(@Nullable final Activity activity) {
        if (activity == null) {
            return;
        }
        ...
        try {
            ...
            for (int i = 0; i < decorView.getChildCount(); i++) {
                ...
                if (childView != null && childView.getWindowToken() != null) {
                    childView.animate().alpha(0).withEndAction(getRemoveViewRunnable(childView));
                }
            }
    ...
    public Alerter setBackgroundColorRes(@ColorRes final int colorResId) {
        if (getAlert() != null && getActivityWeakReference() != null) {
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
                res = getActivityWeakReference().get().getColor(colorResId);
            } else {
                res = getActivityWeakReference().get().getResources().getColor(colorResId);
            }

            getAlert().setAlertBackgroundColor(res);
        }
